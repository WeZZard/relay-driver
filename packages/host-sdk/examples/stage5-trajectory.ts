/**
 * Stage 5 REVIEW-01/REVIEW-02 prep: build trajectory.json for the delivered
...
async function main(): Promise<void> {
  ...
  const wt = await buildTrajectory(packageDir);
  await writeFile(join(packageDir, "trajectory.json"), JSON.stringify(wt, null, 2));
  console.log("trajectory.json written:", wt.steps.length, "steps");
