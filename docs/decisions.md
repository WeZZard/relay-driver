...
This is the authoritative inventory of owner-confirmed decisions. The records below come from the trajectory discussion, the owner's approved trajectory-capture policy on 2026-09-08, and the owner's review-option selections and follow-up ratification on 2026-09-10. Mechanisms without an owner decision remain architect-derived and open to change.
...
- **D2 Environment composition** (owner-set): Compose trajectories with the remote-computer and VM sections of the governing environment policy. Inherit task identity, working directory, ownership, and the evidence-transfer channel; environment allocation and cleanup remain governed there.
- **D3 Real input and accessibility testing** (owner-set): Perform ordinary interactions through real pointer and keyboard input. Accessibility may discover controls, resolve coordinates, and observe state. Direct accessibility activation or value-setting is reserved for a test of that accessibility behavior and must be identified in the evidence. Keyboard-navigation tests use real keystrokes. Keep trajectory-specific instrumentation outside the shipped application.
- **D13 Both remote execution paths** (owner-set, 2026-09-08): Support both operations submitted through SSH and scripts uploaded and then executed remotely. Both paths must correctly record their individual trajectory events and timestamps under the same evidence and media-timing contract. An uploaded script's process start and finish alone do not satisfy its individual-action recording requirement.
...
- **D25 Trajectory terminology** (owner-set, 2026-09-23): The recorded action sequence is called a trajectory; "walkthrough" is retired, and readers accept legacy packages.
